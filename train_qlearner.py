...
from LoadReplayData import load_all_replays, generate_target_tensors, load_all_data, load_data
...
def multi_label_msre(y_true, y_pred):
    # y_true is (batch_size, WIDTH*WIDTH*4)
    ...
    mask = K.cast(K.not_equal(true_flat, 0), dtype='float32')
    prediction = K.sum(K.sum(tf.multiply(mask, pred_flat), axis=-1), axis=-1) # (batch_size)
    ground_truth = K.sum(K.sum(tf.multiply(mask, true_flat), axis=-1), axis=-1) # (batch_size)
    
    squared_error = (ground_truth - prediction)**2
    msre = K.sum(squared_error) / K.cast(batch_size, dtype='float32')
    return msre

def multi_label_msre_pos(y_true, y_pred):
    # y_true is (batch_size, WIDTH*WIDTH)
    # Target input: (batch_size, 2420)
    batch_size = K.shape(y_true)[0]

    #true_shaped = K.reshape(y_true, (batch_size, ORIGINAL_MAP_WIDTH,ORIGINAL_MAP_WIDTH)) #(batch_size, 22, 22, 5)
    true_flat = K.reshape(y_true, (batch_size, ORIGINAL_MAP_WIDTH*ORIGINAL_MAP_WIDTH))
    pred_flat = K.reshape(y_pred, (batch_size, ORIGINAL_MAP_WIDTH*ORIGINAL_MAP_WIDTH))
    
    mask = K.cast(K.not_equal(true_flat, 0), dtype='float32')
    prediction = K.sum(tf.multiply(mask, pred_flat), axis=-1) # (batch_size)
    ground_truth = K.sum(tf.multiply(mask, true_flat), axis=-1) # (batch_size)
...
def prediction_accuracy(y_true, y_pred):
    ...
    correct = K.cast(tf.equal(tf.less(prediction, 0), tf.less(ground_truth, 0)), dtype='float32')
    
    accuracy = K.sum(correct) / K.cast(batch_size, dtype='float32')
    return accuracy
    
def prediction_accuracy_pos(y_true, y_pred):
    # y_true is (batch_size, WIDTH*WIDTH*4)
    # Target input: (batch_size, 2420)
    batch_size = K.shape(y_true)[0]
...
def augment_batch(batch_X, batch_y, batch_size):
    for i in range(batch_size):
        rotation = 0#randint(0, 3)
        flip_vert = 0#randint(0,1)
        ...
        target_value = np.sum(example_tile[example_tile != 0])
        direction = np.argmax(example_direction[example_tile != 0])
        ...
        final_direction[example_tile != 0, direction] = target_value    
...
def frame_generator(training_input_orig, training_target_orig, batch_size, augment=False):
    while True:
        ...
        for batch_index in range(batches):
            ...
            yield batch_X, [tile_target, move_direction_target]
...
def build_model():
    ...
    tile_position = Convolution2D(1, 5, 5, border_mode="same", activation='tanh')(cnn)
    tile_position = Flatten(name='tile_position')(tile_position)
    ...
    move_direction = Convolution2D(4, 5, 5, border_mode="same", activation='tanh')(cnn)
    ...
    model.compile('adam', loss={'tile_position': multi_label_msre_pos, 'move_direction': multi_label_msre},
                  metrics={'tile_position': prediction_accuracy_pos, 'move_direction': prediction_accuracy},
loss_weights={'tile_position': 1, 'move_direction' : 1.0})
    
    return model

def load_model_train_q(dataFolder, modelName):
...
if __name__ == "__main__":
    ...
    training_input, training_target, validation_input, validation_target = load_data('A18-Flobot4000Games3')
    training_input2, training_target2, validation_input2, validation_target2 = load_data('A18-Flobot4000Games4')
    training_input = np.concatenate((training_input, training_input2), axis=0)
    training_target = np.concatenate((training_target, training_target2), axis=0)
    validation_input = np.concatenate((validation_input, validation_input2), axis=0)
    validation_target = np.concatenate((validation_target, validation_target2), axis=0)
    model = build_model()
