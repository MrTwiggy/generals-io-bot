...
var replayPath = process.argv[2];
var destinationPath = replayPath + 'eplay'; // Concatenate 'eplay' so that the extension becomes '.gioreplay'
var input = fs.readFileSync(replayPath);
...
try {
	fs.writeFileSync(destinationPath, JSON.stringify(deserialize(input)));
